pipeline {
    ...
    stages {
        ...
        stage('SAST SonarQube') {
            ...
            steps {
                catchError(buildResult: 'SUCCESS', stageResult: 'FAILURE') {
                    sh 'sonar-scanner -Dsonar.projectKey=dsvw -Dsonar.qualitygate.wait=true -Dsonar.sources=. -Dsonar.host.url=http://192.168.0.116:9000 -Dsonar.token=$SONARQUBE_CREDENTIALS_PSW' 
                }
            }
        }
        ...
        stage('Deploy Docker Image') {
            ...
            steps {
                withCredentials([sshUserPrivateKey(credentialsId: "DeploymentSSHKey", keyFileVariable: 'keyfile')]) {
                    ...
                    sh 'ssh -i ${keyfile} -o StrictHostKeyChecking=no ubuntu@192.168.0.117 docker pull xenjutsu/dsvw:0.1'
                    sh 'ssh -i ${keyfile} -o StrictHostKeyChecking=no ubuntu@192.168.0.117 docker rm --force dsvw'
                    sh 'ssh -i ${keyfile} -o StrictHostKeyChecking=no ubuntu@192.168.0.117 docker run -it --detach --name dsvw --network host xenjutsu/dsvw:0.1'
                }
            }
        }
    }
